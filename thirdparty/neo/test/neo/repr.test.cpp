...
TEST_CASE("repr() some numbers") {
    ...
    double d = 33.02;
    CHECK_THAT(neo::repr(d).string(), Catch::Matchers::Matches("33\\.02.*"));
    float f = 4.1f;
    CHECK_THAT(neo::repr(f).string(), Catch::Matchers::Matches("4\\.1.*f"));
}
...
TEST_CASE("repr() some pointers") {
    ...
    void*       vp  = &i;
    const void* cvp = &i;
    CHECK_THAT(neo::repr(vp).string(), Catch::Matchers::StartsWith("[void* 0x"));
    CHECK_THAT(neo::repr(cvp).string(), Catch::Matchers::StartsWith("[void const* 0x"));

    unknown_thing thing;
    CHECK_THAT(neo::repr(&thing).string(), Catch::Matchers::StartsWith("[unknown-pointer 0x"));
}
